...
export const TabletStockPage = () => {
  ...
  // Auto-clean legacy sold markers: remove per-unit rows with qty <= 0.
  // These rows are leftovers from old sell logic and should not stay in stock.
  useEffect(() => {
    const phonesToClean = inventory.filter(phone => {
      if (!phone.colors || phone.colors.length === 0) return false;
      const hasLegacySoldMarker = phone.colors.some(c => (c.qty || 0) <= 0);
      if (!hasLegacySoldMarker) return false;

      const remainingColors = phone.colors.filter(c => (c.qty || 0) > 0);
      const nextQty = remainingColors.reduce((sum, c) => sum + (c.qty || 0), 0);
      return remainingColors.length !== phone.colors.length || nextQty !== phone.quantity;
    });

    if (phonesToClean.length === 0) return;

    phonesToClean.forEach(phone => {
      const remainingColors = (phone.colors || []).filter(c => (c.qty || 0) > 0);
      const nextQty = remainingColors.reduce((sum, c) => sum + (c.qty || 0), 0);
  ...
  const confirmSell = () => {
    ...
    if (updatedColors && updatedColors.length > 0) {
      ...
      if (!col || col.qty <= 0) return;
      soldColor = col;
      colorRef = col.reference;
      updatedColors = updatedColors
        .map((c, i) => {
          if (i !== sellColorIndex) return c;
    ...
    const newQty = updatedColors && updatedColors.length > 0
      ? updatedColors.reduce((sum, c) => sum + c.qty, 0)
      : Math.max(0, sellingPhone.quantity - 1);
    const phoneUpdates = updatedColors !== undefined
      ? { quantity: newQty, colors: updatedColors }
      : { quantity: newQty };
    updatePhone(sellingPhone.id, phoneUpdates);
    ...
    addSale({
      ...
      phoneRam: soldColor?.ram || sellingPhone.ram,
      phoneStorage: soldColor?.storage || sellingPhone.storage,
      phoneCondition: (soldColor?.condition || sellingPhone.condition) as 'Neuf' | 'Occasion',
